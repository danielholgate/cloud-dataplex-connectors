...
def choose_metadata_type_udf(data_type: str):
    """Choose the metadata type based on PostgreSQL native type."""
    if data_type.startswith("numeric") or data_type.startswith("int") or data_type in ["double precision", "real", "money"]:
        return "NUMBER"
    if data_type.startswith("character varying") or data_type.startswith("text") or data_type.startswith("char"):
        return "STRING"
...
def build_dataset(config, df_raw, db_schema, entry_type):
    ...
    # 1. Alters is_nullable content from True/False to NULLABLE/REQUIRED
