...
def choose_metadata_type_udf(data_type: str):
    ...
    if data_type in ["bigint", "int", "smallint", "tinyint", "bit", "decimal", "numeric", "money", "smallmoney", "float", "real"]:
        return "NUMBER"
    if data_type in ["char", "varchar", "text", "nchar", "nvarchar", "ntext"]:
        return "STRING"
    if data_type in ["date", "datetime", "datetime2", "smalldatetime", "time"]:
        return "DATETIME"
    return "OTHER"
...
def build_dataset(config, df_raw, db_schema, entry_type):
    ...
    # 1. Alters NULLABLE content from 1/0 to NULLABLE/REQUIRED
